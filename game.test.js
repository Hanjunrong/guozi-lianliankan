...
assert(html.indexOf('function handleEliminateAllClick()') !== -1, '一键消除: UI处理函数存在');
assert(html.indexOf('function eliminateAll()') === -1, '一键消除: 不覆盖 game.js 的 eliminateAll');

console.log('\n--- 测试21: 提示连线触发时机与保留策略 ---');
assert(html.indexOf('setupGameUI();\n    startTimer();\n    showHint();') === -1, '提示连线: 开局5秒倒计时开始时不自动提示');
assert(html.indexOf('resetTimer();\n              showHint();') === -1, '提示连线: 连接成功重置倒计时时不自动提示');
assert(html.indexOf('function showHint() {\n      removeHint();') === -1, '提示连线: 生成新提示时不清除旧提示');
assert(html.indexOf('function onBoardClick() {\n      removeHint();') === -1, '提示连线: 点击棋盘时不清除提示');

console.log('\n--- 测试22: 一键消除后倒计时重置条件 ---');
assert(html.indexOf('var beforePairs = window.getPairs();') !== -1, '一键消除: 记录消除前剩余对数');
assert(html.indexOf('var eliminatedCount = beforePairs - pairs;') !== -1, '一键消除: 计算本次消除数量');
assert(html.indexOf('if (eliminatedCount > 1) {\n        resetTimer();\n      }') !== -1, '一键消除: 消除超过1对时重置倒计时');
...
assert(html.indexOf('function renderAxisLabels()') !== -1, '坐标轴: 存在坐标渲染函数');
assert(html.indexOf('for (var x = 1; x <= COLS; x++)') !== -1, '坐标轴: 横坐标从1开始');
assert(html.indexOf('for (var y = 1; y <= ROWS; y++)') !== -1, '坐标轴: 纵坐标从1开始');

console.log('\n--- 测试24: 游戏结束弹窗与重开按钮 ---');
assert(html.indexOf('id="finalScore"') !== -1, '结束弹窗: 展示最终分数区域');
assert(html.indexOf('id="restartBtn"') !== -1, '结束弹窗: 存在重新开始按钮');
assert(html.indexOf('function showGameOver()') !== -1, '结束弹窗: 存在结束处理函数');
assert(html.indexOf('function restartGame()') !== -1, '结束弹窗: 存在重新开始函数');
assert(html.indexOf('function restartGame() {\n      removeHint();') !== -1, '结束弹窗: 重新开始前清理历史提示线');
assert(html.indexOf('function isBoardCleared()') !== -1, '结束判定: 存在棋盘清空检测函数');
assert(html.indexOf('if (pairs === 0)') !== -1, '结束判定: remaining为0时检查状态');
assert(html.indexOf('<span id="timer">⏰ 5</span>') !== -1, '倒计时: 初始展示为5秒');
assert(html.indexOf('timerValue = 5;') !== -1, '倒计时: 所有重置点统一为5秒');
assert(html.indexOf('function showGameOver() {\n      if (timerInterval) {\n        clearInterval(timerInterval);\n        timerInterval = null;\n      }') !== -1, '倒计时: 通关后暂停计时器');

console.log('\n--- 测试25: 提示线交互、连线坐标与音效 ---');
assert(html.indexOf('function consumeHintOnAction()') !== -1, '提示线: 存在点击后消费提示函数');
assert(html.indexOf('if (hintLine) {\n        removeHint();\n        resetTimer();\n      }') !== -1, '提示线: 点击后消失并重置倒计时');
assert(html.indexOf("document.querySelectorAll('.line:not(.hint)')") !== -1, '提示线: 清理临时线时保留黄色提示线');
assert(html.indexOf('function handleClick(row, col) {\n      consumeHintOnAction();') !== -1, '提示线: 鼠标点击触发后才消费提示线');
assert(html.indexOf('function getBoardMetrics()') !== -1, '连线: 存在棋盘度量函数');
assert(html.indexOf('originX') !== -1 && html.indexOf('stepX') !== -1, '连线: 使用网格度量计算中心坐标');
assert(html.indexOf('transform: scale(1.5);') !== -1, '悬停: 格子放大0.5倍');
assert(html.indexOf('transition: transform 0.5s ease') !== -1, '悬停: 放大动画时长0.5s');
assert(html.indexOf('window.playSuccessSfx') !== -1, '音效: 成功时触发成功音');
assert(html.indexOf('window.playFailSfx') !== -1, '音效: 失败时触发失败音');
...
assert(html.indexOf('function handleToggleSfx()') !== -1, '音效: 页面存在音效开关处理函数');
assert(html.indexOf('function refreshSfxButton()') !== -1, '音效: 页面存在图标刷新函数');
assert(html.indexOf("window.isSfxEnabled() ? '🔊' : '🔇'") !== -1, '音效: 使用图标显示开关状态');
